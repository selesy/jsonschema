...
import (
	...
	"github.com/selesy/jsonschema/examples"
)
...
func prepareCommentReflector(t *testing.T, opts ...CommentOption) *Reflector {
	t.Helper()
	r := new(Reflector)
	err := r.AddGoComments("github.com/selesy/jsonschema", "./examples", opts...)
...
func prepareCustomCommentReflector(t *testing.T) *Reflector {
	...
	err := r.AddGoComments("github.com/selesy/jsonschema", "./examples")
	require.NoError(t, err, "did not expect error while adding comments")
	return r
}
